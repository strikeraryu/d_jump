...
plant_jump = [pg.image.load('images/plant_jump/plant_jump (1).png'),pg.image.load('images/plant_jump/plant_jump (2).png'),pg.image.load('images/plant_jump/plant_jump (3).png'),pg.image.load('images/plant_jump/plant_jump (4).png')]
plant_jump = [pg.transform.scale(img,(64,68)) for img in plant_jump]
plant_run = [pg.image.load('images/plant_run/plant_run (1).png'),pg.image.load('images/plant_run/plant_run (2).png'),pg.image.load('images/plant_run/plant_run (3).png'),pg.image.load('images/plant_run/plant_run (4).png')]
...
class crate_obj(object):
    ...
    def draw(self):
        win.blit(crate_img,(self.x,self.y))
...
while run:
    ...
    if len(crates) <= max_crate:
        if len(crates)!=0:
            new_crate = crate_obj(crates[-1].x + random.randint(300,400),obj_y+20)
            crates.append(new_crate)
        if len(crates)==0:
            new_crate = crate_obj(700,obj_y+20)
            crates.append(new_crate)
